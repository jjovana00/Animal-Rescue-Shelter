$(document).ready(function() {
    ...
    function izlistajZivotinje(){
        for(let i=0; i< zivotinje.length; i++){
            ...
            paragraf=$('<p class="card-text"> Description: <label>'+ zivotinje[i].opis +
            '</label><br>Age: <label class="g">' + zivotinje[i].godine + '</label><br>Weight: ' + zivotinje[i].tezina + '<br></p>');
            $(cardbody).append(paragraf);
            $(text).append(cardbody);
            profil=$('<a> <button type="button" class="btn btn-dark profil" style="margin-left: 2.5%;" id="'+ zivotinje[i].id+'">Go to profile</button></a>');
            $(text).append(profil);
    ...
    $("#pretrazi").click(function () {
        ...
        if (krit != "") {
            if(val==1) {
                for (let i = 0; i < arr.length; i++) {
                    let obj = $(arr[i]).find(".row").find(".col-md-8").find(".card-body").find(".card-text").find(".g");
                ...
                for (let i = 0; i < arr.length; i++) {
                    let obj = $(arr[i]).find(".row").find(".col-md-8").find(".card-body").find("h5");
                    let ime = $(obj).text();
                    ime = ime.toLowerCase();
