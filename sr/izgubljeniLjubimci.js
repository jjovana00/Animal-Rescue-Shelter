$(document).ready(function() {
    ...
    function izlistajOglase(){
        for(let i=0; i< oglasi.length; i++){
            ...
            paragraf=$('<p class="card-text oglas_'+ oglasi[i].id+'"><b>Opis: </b><label>'+ oglasi[i].opis +
            '</label><br><b>Telefon: </b>' + oglasi[i].telefon + '<br></p>');
            $(cardbody).append(paragraf);
            let download = $("<br><button class='btn btn-dark'>Skini PDF</button>");
            ...
            $(column3).append('<div><br><b>Dodaj novi komentar</b><br><textarea cols="40" rows="3" id="kom_' + oglasi[i].id + '"></textarea><br><button class="btn btn-dark dodaj_kom" id="' + oglasi[i].id + '">Dodaj</button></div>');
            $(rowdiv).append(photodiv);
            $(rowdiv).append(text);
            $(rowdiv).append(column3);
            let rowdiv2=$('<div class="row g-0 mt-1 mb-4 mx-3"></div>');
            let column1=$('<div class="col-md-8"><h5>Komentari:</h5></div>');
            ...
            if(nemaKomentara == true) {
                let tr = $("<tr></tr>");
                $(tr).append("<td colspan='2'>Ne postoje komentari za ovaj oglas.</td>");
                $(table).append(tr);
            } 
